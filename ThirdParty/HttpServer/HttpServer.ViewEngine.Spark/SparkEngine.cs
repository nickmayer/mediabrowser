...
namespace HttpServer.ViewEngine.Spark
{
    ...
    public class SparkEngine : IViewEngine
    {
        ...
        public void Render(IControllerContext context, IViewData viewData, TextWriter writer)
        {
            ...
            SparkView view;
            Mapping mapping = GetMapping(context.RequestContext.Request.IsAjax, viewUri);
            if (mapping == null || true)
                view = GenerateView(context, viewUri, viewData);
            else
                view = mapping.Create(viewData.GetHashCode()) ?? GenerateView(context, viewUri, viewData);
