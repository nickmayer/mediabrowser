...
namespace HttpServer.Mvc.Controllers
{
    ...
    public class ControllerDirector : IControllerDirector
    {
        ...
        private void FindControllerName(Type type)
        {
            ...
            if (string.IsNullOrEmpty(_name))
            {
                ...
                if (_name.EndsWith("Controller"))
                    _name = _name.Replace("Controller", string.Empty);
            }
            ...
            _name = _name.ToLower();
            _logger.Debug("Selected controller name '" + _name + "' with uri '" + _uri + "'");
        }

        private MethodInfo GetMethod(Method httpMethod, string actionName)
        {
        ...
        private void MapMethod(MethodInfo info)
        {
            ...
            if (isFound)
            {
                _logger.Trace("Mapped " + _name + "." + methodName.ToLower() + " to " + verb);
            ...
            foreach (object customAttribute in info.GetCustomAttributes(false))
            {
                ...
                mapping.IsMethodsSpecified = true;
                foreach (Method method in attr.Methods)
                {
                    _logger.Trace("Mapped " + _name + "." + methodName.ToLower() + " to " + method);
            ...
            {
                _logger.Trace("Mapped " + _name + "." + methodName.ToLower() + " to ALL verbs");
                mapping.Add(verb, info);
            }
        }
        ...
        /// </code>
        /// </example>
        private bool TryGetVerb(ref string name, out Method verb)
        {
            name = name.ToLower();
            foreach (string httpMethod in Enum.GetNames(typeof (Method)))
            {
                if (!name.EndsWith(httpMethod.ToLower())) continue;
                verb = (Method) Enum.Parse(typeof (Method), httpMethod);
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<ControllerRoute> GetRoutings()
        {
            ...
            var routings = new List<ControllerRoute>();
            /*
            foreach (MethodInfo method in _methods.Values)
            {
                routings.Add(new ControllerRoute(this, prefix + method.Name));
            }
			*/
            return routings;
        }
        ...
        public object Process(IControllerContext context, out Controller controller)
        {
            ...
            if (context.UriSegments.Length < _uriSegments)
                throw new NotFoundException("No action specified for controller '" + _name + "'.");
        ...
        {
            private readonly Dictionary<Method, MethodInfo> _mappings = new Dictionary<Method, MethodInfo>();
            ...
            public void Add(Method method, MethodInfo info)
            {
    ...
    public class DirectorContext : IDisposable
    {
        public ControllerDirector Director { get; set; }
